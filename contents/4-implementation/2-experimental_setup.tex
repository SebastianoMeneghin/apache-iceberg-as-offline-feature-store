...
\begin{minipage}{\textwidth}
    \begin{python}[caption={[Measuring latency using the time difference] A simple time difference approach the time to read from Iceberg table stored on \gls{HopsFS}.}, label={lst:ch4_timetime}, basicstyle=\small]
    import time
    from pyiceberg.catalog import load_catalog
